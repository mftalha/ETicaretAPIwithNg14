...
public class AuthService : IAuthService
{
    ...
    public async Task PasswordResetAsync(string email)
    {
        ...
        if(user != null)
        {
            //bu user'a özel bir resettoken üretiyoruz ve elde ediyoruz
            // bu methodu kullanabilmek için AddDefaultTokenProviders()'ı ioc'ye dahil etmek lazım
            string resetToken = await _userManager.GeneratePasswordResetTokenAsync(user);

            ////reset token'ı byte çeviriyoruz
            //byte[] tokenBytes = Encoding.UTF8.GetBytes(resetToken);
            //// reset oken içinde tırnak gibi karekterler olabilir ama biz bunları url üzerinden taşıyamayız o yüzden url üzerinden taşıyabileceğimiz formata çeviriyoruz. => http protokole uygun hale getiriyoruz
    ...
    public async Task<bool> VerifyResetTokenAsync(string resetToken, string userId)
    {
        ...
        if(user != null)
        {
            //// Encode ederek cliente yolladığım resetToken'ı tekrar decode ediyorum kontrol edebilmek için => gönderdiğim reset token ile gelen reset token aynımı
            //byte[] tokenBytes = WebEncoders.Base64UrlDecode(resetToken);
            //// reset token'ın ilk oluiturulşmuş halini elde ediyoruz : aynı reset token ise
            //resetToken = Encoding.UTF8.GetString(tokenBytes);

            resetToken = resetToken.UrlDecode();

            //burada ilgili reset token doğrumu ve o kullanıcı içinmi o reset token gönderilmişti diye kontrolünü sağlıyoruz.
            // => PasswordResetTokenProvider , "ResetToken" => ile hangi token'ın doğrulamasını yapacağımızı söylüyoruz
            // doğrulanıyorsa true ; doğrulanmıyorsa false
